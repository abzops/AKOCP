import { AlertCircle, ArrowDownLeft, ArrowUpRight, Check, CircleDollarSign, Clock3, FileImage, Plus, ReceiptText, Search, ShieldCheck, WalletCards, X } from 'lucide-react'
...
function WithdrawalModal({ open, onClose, balance }: { open: boolean; onClose(): void; balance: number }) {
  ...
  return <Modal open={open} onClose={close} title="Request withdrawal" description="Founder approval is required before any manual transfer." footer={<><Button variant="ghost" onClick={close}>Cancel</Button><Button form="withdrawal-form" type="submit" disabled={balance <= 0} loading={busyAction === 'request-withdrawal'}>Send request</Button></>}><form id="withdrawal-form" onSubmit={submit} className="form-stack"><div className="available-note"><WalletCards size={18} /><span>Available wallet</span><strong>{formatCurrency(balance)}</strong></div>{balance <= 0 ? <div className="form-notice warning"><AlertCircle size={16} /> The brand wallet currently has ₹0 available. New customer payments must be confirmed before requesting withdrawals.</div> : <Input label="Amount" type="number" min="1" max={Math.max(1, balance)} step="1" value={amount} onChange={(event) => setAmount(event.target.value)} required />}{balance > 0 && <Select label="Reason" value={reason} onChange={(event) => setReason(event.target.value as WithdrawalReason)}>{withdrawalReasons.map((item) => <option key={item} value={item}>{withdrawalReasonLabels[item]}</option>)}</Select>}{balance > 0 && <Textarea label="Notes" value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Add context for the Founder…" rows={3} />}<div className="form-notice"><ShieldCheck size={16} /> Submitting this request does not move money or reduce the wallet.</div></form></Modal>
}
