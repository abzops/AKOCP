...
import { getExpenseBreakdown, getLanguageOrders, getMonthlyRevenue, getServiceRevenue } from '../lib/analytics'
...
export function AnalyticsPage() {
  ...
  const periodRevenue = paidOrders.reduce((sum, order) => sum + order.price, 0)
  const averageOrder = paidOrders.length ? periodRevenue / paidOrders.length : 0
  ...
  const exportReport = () => downloadCsv(`AKOCP-analytics-${range}-${new Date().toISOString().slice(0, 10)}.csv`, [
    ...
    { Metric: 'Average order value', Value: averageOrder },
  ...
  return (
    <div className="page-stack analytics-page">
      ...
      <div className="stats-grid analytics-stats"><MoneyStat label="Period revenue" value={periodRevenue} icon={<CircleDollarSign size={21} />} tone="accent" /><StatCard label="Paid orders" value={paidOrders.length} icon={<ShoppingBag size={21} />} /><MoneyStat label="Average order" value={averageOrder} icon={<TrendingUp size={21} />} /><StatCard label="Best service" value={bestService?.name ?? '—'} icon={<Crown size={21} />} trend={bestService ? formatCurrency(bestService.revenue) : 'No sales'} /></div>
