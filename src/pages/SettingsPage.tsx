...
function TeamSettings() {
  ...
  const toggleExpenseAccess = async (member: Profile) => execute(`expense-permission-${member.id}`, `${member.full_name}'s expense access updated`, () => service.setExpensePermission(member.id, !member.can_submit_expenses, profile), ['profiles'])
  return <Card className="settings-panel"><div className="settings-heading"><span><UserCog size={21} /></span><div><h2>Team access</h2><p>Founder controls roles and may grant expense submission to a specific Operations account such as Roshan.</p></div></div><div className="team-list">{snapshot.profiles.map((member) => <div key={member.id}><span className="avatar large">{initials(member.full_name)}</span><div><strong>{member.full_name}{member.id === profile.id && <Badge tone="yellow">You</Badge>}</strong><small>{member.email}</small></div>{member.role === 'operations' && <label className="expense-access-toggle"><input type="checkbox" checked={Boolean(member.can_submit_expenses)} disabled={busyAction === `expense-permission-${member.id}`} onChange={() => void toggleExpenseAccess(member)} /><span>Submit expenses</span></label>}<Badge tone={member.role === 'founder' ? 'yellow' : 'blue'}>{humanize(member.role)}</Badge><select aria-label={`Role for ${member.full_name}`} value={member.role} disabled={member.id === profile.id || busyAction === `profile-${member.id}`} onChange={(event) => setChange({ member, role: event.target.value as Role })}><option value="founder">Founder</option><option value="operations">Operations</option></select></div>)}</div><div className="permissions-grid"><div><ShieldCheck size={18} /><strong>Founder</strong><p>Reviews every submitted expense before it reaches the wallet, including Founder submissions.</p></div><div><UserCog size={18} /><strong>Operations</strong><p>Only explicitly enabled accounts can submit expenses; all others retain normal Operations access.</p></div></div><ConfirmDialog open={Boolean(change)} onClose={() => setChange(null)} onConfirm={() => void confirm()} title="Change team role?" message={`${change?.member.full_name ?? 'This member'} will receive ${change?.role === 'founder' ? 'full Founder permissions' : 'Operations permissions'} on their next action.`} confirmLabel="Change role" loading={Boolean(change && busyAction === `profile-${change.member.id}`)} /></Card>
}
