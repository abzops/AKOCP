...
export const supabaseDataService: DataService = {
  async loadSnapshot(userId: string) {
    const [services, profiles, customers, tracks, orders, recordedSales, payments, expenses, withdrawals, notifications, audits, transactions] = await Promise.all([
    ...
    const error = [services, profiles, customers, tracks, orders, recordedSales, payments, expenses, withdrawals, notifications, audits, transactions].find((result) => result.error)?.error
    assertNoError(error)
    return {
